...
*/
export async function listBorrows(c: Context<{ Bindings: { HYPERDRIVE: Hyperdrive } }>) {
...
*/
export async function getBorrowById(c: Context<{ Bindings: { HYPERDRIVE: Hyperdrive } }>) {
  const sql = await pgClient(c.env.HYPERDRIVE.connectionString);
...
*/
export async function getBorrowByUserId(c: Context<{ Bindings: { HYPERDRIVE: Hyperdrive } }>) {
  const sql = await pgClient(c.env.HYPERDRIVE.connectionString);
  const userId = Number(c.req.param("userId"));
...
*/
export async function getBorrowByBookId(c: Context<{ Bindings: { HYPERDRIVE: Hyperdrive } }>) {
  const sql = await pgClient(c.env.HYPERDRIVE.connectionString);
  const bookId = Number(c.req.param("bookId"));
 ...
 * Create a new borrow
*/
export async function createBorrow(c: Context<{ Bindings: { HYPERDRIVE: Hyperdrive } }>) {
  const sql = await pgClient(c.env.HYPERDRIVE.connectionString);
